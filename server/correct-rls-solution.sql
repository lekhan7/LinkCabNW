-- ================================================================
-- CORRECT RLS SOLUTION - PROPER SYSTEM CATALOG COLUMNS
-- ================================================================
-- This script uses correct column names from pg_policy system catalog

-- Step 1: Disable RLS on all tables (this removes all policies)
...
-- Step 2: Drop any remaining policies using correct system catalog columns
DO $$
DECLARE
    policy_rec RECORD;
BEGIN
    FOR policy_rec IN 
        SELECT polname::TEXT, classname::TEXT 
        FROM pg_policy 
        WHERE classname IN (
            'users', 'admins', 'app_settings', 'preferred_travel_places', 'preferred_locations',
            'announcements', 'announcement_participants', 'announcement_completion_status',
            'rides', 'ride_shares', 'payments', 'notifications', 'ratings', 'reviews',
            'favorite_routes', 'connection_requests', 'otps'
        )
    LOOP
        BEGIN
            EXECUTE format('DROP POLICY IF EXISTS %I ON %s', 
                       quote_ident(policy_rec.polname), 
                       quote_ident(policy_rec.classname));
